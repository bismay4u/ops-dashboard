...
Self-hosted, multi-dashboard app launcher — a cards-and-bookmarks home page with IP-based 
auto-login, live status badges, an admin console, and omnisearch.

Designed to replace several separately-maintained static dashboard pages with one
...
sample "Main Dashboard". Log in at `/index.html`, then go to `/admin.html` to add
your real dashboards, categories, apps, and bookmarks.

## Multi-role users, role-controlled dashboards/items, and custom sections (v4)

This is a bigger shift than earlier updates: visibility moved from a single
...
- **One dashboard tool instead of four** — each of your office dashboards becomes a
  ...
  from one place instead of four separate codebases.
- **JSON rules file, no auto-discovery** — categories/items live in SQLite (so the
  ...
  Nothing auto-discovers services from Nginx Proxy Manager or Docker — every tile is
...
- **Users managed by IP, falling back to login** — every request is checked against
  ...
  desk/workstation won't need to log in again). Admins can also add, fix, or remove
  IP↔user mappings by hand under Admin → IP Mappings — useful for shared kiosks or
  static desk IPs, or if someone's IP changes via DHCP.
...
## Notes / things to decide before rolling out to all 4 offices
...
- There's no rate limiting on `/api/auth/login` yet; add one (e.g. `express-rate-limit`)
  before exposing this outside your office network.
